...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport
        ref={ref}
        className={cn(
            'fixed bottom-space-05 z-[10000000000000] flex max-h-screen w-full flex-col p-4 sm:bottom-10 sm:left-1/2  sm:top-auto lg:max-w-[500px] ltr:sm:translate-x-1/2 rtl:sm:-translate-x-1/2',
            className,
        )}
        {...props}
    />
...
const ToastTitle = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Title>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Title>>(
    ({ className, ...props }, ref) => (
        <ToastPrimitives.Title
            ref={ref}
            className={cn('text-body-01 font-semibold group-[.destructive]:text-error group-[.success]:text-success', className)}
            {...props}
        />
    ),
)
