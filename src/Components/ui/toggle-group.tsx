...
>(({ className, variant, size, colors, rounded, children, ...props }, ref) => (
    <ResponsiveScroll buttonClassName="bg-background">
        <ToggleGroupPrimitive.Root ref={ref} className={cn('flex items-center justify-center gap-space-01', className)} {...props}>
            <ToggleGroupContext.Provider value={{ variant, size, colors, rounded }}>{children}</ToggleGroupContext.Provider>
...
>(({ className, children, variant, size, colors, rounded, ...props }, ref) => {
