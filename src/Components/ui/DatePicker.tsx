...
const datePickerVariants = cva(
    ...
    `,
  {
    variants: {
      variant: {
        default:
          "bg-form-field-background-darker  enabled:hover:border-form-field-border-default",
        outline:
        ...
        lighter:
          "bg-form-field-background-lighter  enabled:hover:border-form-field-border-default",
      },
      rounded: {
        default: "rounded",
        full: "rounded-4",
...
export const DatePicker = ({
  ...
  const [toggleCalendar, setToggleCalendar] = useState<"hijri" | "gregorian">(
    defaultCalendar,
  );
  const dateValue =
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen} modal={true}>
      <PopoverTrigger asChild>
        <button
          ...
          role="date"
          className={cn(
            datePickerVariants({ variant, colors, size, rounded }),
            className,
          )}
        ...
        >
          {dateValue ? (
            dateValue.format(
              momentFormat?.(lang, toggleCalendar) || dateFormate,
            )
          ) : (
      ...
      <PopoverContent
        className="w-auto rounded-2 border-border-secondary p-0 shadow-2xl"
        dir={dir}
      >
        <>
          {showSwitch && (
            <div className=" p-space-04">
              <div className="flex w-full items-center justify-between gap-2 border-b border-border-secondary pb-space-04 pt-space-02">
                ...
                <Switch
                  onCheckedChange={() =>
                    setToggleCalendar(
                      toggleCalendar === "hijri" ? "gregorian" : "hijri",
                    )
                  }
                  dir={dir}
